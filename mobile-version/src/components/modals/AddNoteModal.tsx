...
interface AddNoteModalProps {
  ...
  existingNotes?: Note[];
  onAddNote?: (appointmentId: string, noteText: string) => void;
}
...
const AddNoteModal = ({
  ...
  const handleAddNote = () => {
    ...
    if (onAddNote) {
      onAddNote(appointmentId, noteText.trim());
    } else {
      // Mock save - in real app, this would call an API
      console.info("Saving note for appointment", appointmentId, noteText.trim(), uploadedFile?.name);
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md w-[calc(100%-2rem)] p-0 gap-0 rounded-2xl bg-white shadow-lg max-h-[85vh] overflow-y-auto flex flex-col [&>button]:hidden">
        {/* Header */}
